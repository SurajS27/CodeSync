...
from app.schemas.repository import RepositoryCreate, RepositoryResponse, RepositoryCreateResponse
...
@router.post("/create", response_model=RepositoryCreateResponse, status_code=status.HTTP_201_CREATED)
async def create_repository(
    ...
    """
    logger.info(f"Repository creation request received from user: {current_user.github_username}")

    # 1. Check local database
    db_repo = await RepositoryService.get_repository_by_name(db, current_user.id, repo_in.repo_name)

    # 2. Check GitHub API
    github_exists = await GitHubRepositoryService.check_github_repository_exists(current_user, repo_in.repo_name)

    if db_repo and github_exists:
        logger.info(f"Repository '{repo_in.repo_name}' already exists in DB and GitHub. Returning existing.")
    ...
    if github_exists:
        ...
        logger.info(f"Repository '{repo_in.repo_name}' exists on GitHub but not in database. Importing metadata.")
        repo_data = await GitHubRepositoryService.fetch_github_repository_details(current_user, repo_in.repo_name)

        github_created = datetime.fromisoformat(repo_data["created_at"].replace("Z", "+00:00"))
        github_updated = datetime.fromisoformat(repo_data["updated_at"].replace("Z", "+00:00"))
        ...
        if db_repo:
            new_repo = await RepositoryService.update_repository_record(db, db_repo.id, new_repo_data)
        else:
            new_repo = await RepositoryService.create_repository_record(db, current_user.id, new_repo_data)

        # Trigger bootstrap on imported repository
        try:
            await RepositoryBootstrapService.bootstrap_repository(db, current_user, new_repo)
            ...
            logger.error(f"Failed to bootstrap imported repository '{new_repo.repo_full_name}': {str(e)}")
            await db.refresh(new_repo)
            return {
                "repository": new_repo,
    ...
    unique_name = await GitHubRepositoryService.generate_unique_repository_name(db, current_user, repo_in.repo_name)
    ...
    try:
        ...
        logger.error(f"Post-creation auto-bootstrap failed for repository '{new_repo.repo_full_name}': {str(e)}")
        await db.refresh(new_repo)
        return {
            "repository": new_repo,
            "bootstrap_status": BootstrapStatus.FAILED,
...
async def bootstrap_repository_manually(
    ...
    """
    repo = await RepositoryAccessService.verify_repository_owner(db, current_user.id, repository_id)
    try:
        await RepositoryBootstrapService.bootstrap_repository(db, current_user, repo, force=force)
...
async def delete_repository(
    ...
    """
    await RepositoryAccessService.verify_repository_owner(db, current_user.id, repository_id)
