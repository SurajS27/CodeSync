...
class LeetCodeSyncService:
    ...
    async def sync_leetcode_submission(
        db: AsyncSession, user: User, request: LeetCodeSyncRequest
    ) -> dict:
        ...
        repo = await RepositoryAccessService.verify_repository_owner(
            db=db, user_id=user.id, repository_id=request.repository_id
        )
        ...
        try:
            ...
            sync_record = await SyncHistoryService.update_sync_record(
                db=db, sync_id=sync_record.id, status=SyncStatus.RUNNING
            )
            ...
            solution_filename = PathGenerationService.generate_solution_filename(
                request.language
            )
            ...
            readme_sha = await GitHubContentService.check_file_exists(
                user=user, repo_full_name=repo.repo_full_name, path=readme_path
            )

            solution_sha = await GitHubContentService.check_file_exists(
                user=user, repo_full_name=repo.repo_full_name, path=solution_path
            )
            ...
            solution_commit_msg = (
                f"code: sync solution for {request.problem_title} in {request.language}"
            )
