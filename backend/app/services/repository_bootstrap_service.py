...
class RepositoryBootstrapService:
    ...
    async def bootstrap_repository(
        ...
        if (
            ...
            logger.info(f"Repository '{repo.repo_full_name}' is already successfully bootstrapped. Bypassing execution.")
            return True

        logger.info(f"Starting repository bootstrap sequence for: {repo.repo_full_name} (force={force})")

        # 2. Transition status to RUNNING, set attempt timestamp, and clear previous errors
        await RepositoryService.update_repository_record(db, repo.id, {
        ...
        try:
            ...
            for path, content in files_to_create.items():
                sha = await GitHubContentService.check_file_exists(user, repo.repo_full_name, path)
            ...
            await db.refresh(repo)
            logger.info(f"Repository bootstrap completed successfully for: {repo.repo_full_name}")
            return True

        except Exception as e:
            # Truncate error message to 2000 characters to prevent oversized database writes
            error_message = str(e)[:2000]
            logger.error(f"Error provisioning paths during bootstrap of '{repo.repo_full_name}': {error_message}")
            ...
            try:
                ...
                logger.error(f"Failed to persist failure status for '{repo.repo_full_name}': {str(db_err)}")
