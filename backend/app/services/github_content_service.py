...
class GitHubContentService:
    ...
    async def get_file_metadata(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 404:
                return None
            logger.warning(f"GitHub contents check returned status {response.status_code} for path: {path}")
            return None
    ...
    async def check_file_exists(
        ...
        metadata = await GitHubContentService.get_file_metadata(user, repo_full_name, path)
    ...
    async def create_or_update_file(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 403:
                logger.error("GitHub API returned rate limits or permissions block on content write.")
                raise HTTPException(
            ...
            if response.status_code not in (200, 201):
                logger.error(f"Failed to write file. Status: {response.status_code} - Body: {response.text}")
                raise HTTPException(
