...
class GitHubRepositoryService:
    ...
    async def check_github_repository_exists(user: User, repo_name: str) -> bool:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 200:
                logger.info(
                    f"Repository {owner}/{repo_name} verified as existing on GitHub."
                )
                return True
            return False
    ...
    async def generate_unique_repository_name(
        db: AsyncSession, user: User, base_name: str
    ) -> str:
        ...
        while True:
            ...
            db_repo = await RepositoryService.get_repository_by_name(
                db, user.id, candidate_name
            )
            if db_repo is None:
                # 2. Validate GitHub API check
                exists_on_github = (
                    await GitHubRepositoryService.check_github_repository_exists(
                        user, candidate_name
                    )
                )
    ...
    async def create_github_repository(
        user: User, repo_name: str, is_private: bool
    ) -> dict:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 422:
                logger.warning(
                    f"Repository creation failed due to duplicate name on GitHub: {repo_name}"
                )
                raise HTTPException(
            ...
            if response.status_code != 201:
                logger.error(
                    f"GitHub repository creation failed. Status: {response.status_code} - Body: {response.text}"
                )
                raise HTTPException(
    ...
    async def fetch_github_repository_details(user: User, repo_name: str) -> dict:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Failed to fetch GitHub repository details. Status: {response.status_code}"
                )
                raise HTTPException(
                    status_code=status.HTTP_502_BAD_GATEWAY,
